/**
 * Seed / migration-reference editorial copy for the KI / AI page.
 * NOT used at runtime — runtime uses Sanity via getEmptyAiPageContent + mergeSanityAi.
 *
 * Canonical migration source: studio/schemaTypes/aiDefaults.ts
 * (scripts/migrate-ai-page-texts.ts). This module mirrors that copy for tests.
 *
 * No static architecture / 3D / social-marketing image paths.
 */
import type { Locale } from "@/types/i18n";
import type { HomepageMedia } from "@/types/homepage";
import { getClientLogos } from "@/lib/content/clients";
import { localizePathname } from "@/lib/i18n/config";
import type { AiPageContent } from "@/lib/content/ai-page";
...
/**
 * Approved DE/EN copy for migration alignment / tests only.
 * Do not import from page rendering.
 */
export function getAiPageSeedContent(locale: Locale): AiPageContent {
  const contact = localizePathname("/contact", locale);
  const logos = getClientLogos();
  const media: HomepageMedia = { ...EMPTY_MEDIA };
