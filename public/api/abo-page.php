...
$ch = curl_init($url);
if ($ch === false) {
  proxy_client_error(500, 'abo-page', 'curl_init failed');
}
...
if ($responseBody === false || $statusCode < 200 || $statusCode >= 300) {
  proxy_client_error(502, 'abo-page', 'Sanity request failed', [
...
if (!is_array($decoded) || !array_key_exists('result', $decoded)) {
  proxy_client_error(502, 'abo-page', 'Unexpected Sanity response shape');
}

$result = $decoded['result'];
if ($result === null) {
  proxy_client_error(404, 'abo-page', 'Document not found');
}
